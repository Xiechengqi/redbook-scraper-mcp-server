...
if __name__ == "__main__":
    ...
    # 检查是否通过环境变量指定运行模式
    run_mode = os.getenv("RUN_MODE", "both")  # 默认同时运行
    
    if run_mode == "api":
        # 只运行FastAPI服务器
        logger.info("启动FastAPI服务器...")
        run_fastapi_server_process()
    elif run_mode == "mcp":
        # 只运行MCP服务器
        ...
        # 同时运行两个服务器（使用多进程）
        logger.info("同时启动FastAPI和MCP服务器...")
        
        # 创建进程
        fastapi_process: multiprocessing.Process | None = multiprocessing.Process(
            target=run_fastapi_server_process,
            name="FastAPI-Server"
        )
        mcp_process: multiprocessing.Process | None = multiprocessing.Process(
        ...
        def signal_handler(sig, frame):
            ...
            for process in (fastapi_process, mcp_process):
                if process is None:
                    continue
                if process.is_alive():
                    process.terminate()
        ...
        try:
            ...
            logger.info(f"FastAPI服务器进程已启动 (PID: {fastapi_process.pid})")
            
            mcp_process.start()
            logger.info(f"MCP服务器进程已启动 (PID: {mcp_process.pid})")
